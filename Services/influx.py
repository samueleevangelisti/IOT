...
class Influx:
    ...
    def delete_forecasting_arima_all(self):
        ...
        try:
            self.delete_api.delete('1970-01-01T00:00:00Z', (datetime.now() + timedelta(days=365)).timestamp() * 1000000000, '_measurement="forecasting"', bucket=self.config['bucket']['forecasting']['arima'], org=self.config['organization'])
